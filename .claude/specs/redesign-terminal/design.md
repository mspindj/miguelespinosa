...
2. **New code lives in `src/site/`.** It holds the design system (`site.css`, every rule either under the root class `.tb` or on a `tb-`-prefixed class that only new code uses; no bare element or unprefixed class selectors), the shell (layout, header, status bar, shortcuts dialog, keyboard layer, Lenis), `ascii/` (the vanilla three renderer), and `content/` (typed data).
...
- `cases.ts`: one `DecisionRecord` per case, holding the full content of today's page (R2.1). Mapping from today's sections:
  ...
  - IMPLEMENTATION ← what we built / enablers / BBVA products (challenge, decision, result). Exception: Design Transformation's four pillars sit under DECISION, because that four-pillar structure was the decision itself.
...
- One composed sentence from existing facts: the BBVA summary on the records table (approved by Miguel, 2026-09-24).
